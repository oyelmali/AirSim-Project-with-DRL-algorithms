...
class DroneEnvironment(py_environment.PyEnvironment):
  ...
  def __init__(self, enable_wind=False, randomize_initial_pose=False, save_path=None):
    ...
    self._action_spec = array_spec.BoundedArraySpec(shape=(), dtype=np.float32, name='action', minimum=0.0, maximum=1.0) # Action space
  ...
  '''
  def move(self, action, duration=0.002, continuous=True):
    ...
    if continuous == True: # sürekli hareketler -> kontrol döngüsü : ağ çıkarımı -> eylemi eşzamansız olarak gerçekleştir -> ...
      ...
      if(action < 0.20):
         self.client.moveByMotorPWMsAsync(front_right_pwm=1, rear_left_pwm=1, front_left_pwm=1, rear_right_pwm=1,duration=duration) # Move up
      elif(action < 0.40):
        self.client.moveByMotorPWMsAsync(front_right_pwm=0, rear_left_pwm=0, front_left_pwm=0, rear_right_pwm=0,duration=duration) # Move down
      elif(action < 0.60):
         self.client.moveByMotorPWMsAsync(front_right_pwm=1, rear_left_pwm=0, front_left_pwm=0, rear_right_pwm=1,duration=duration) # Move left
      elif(action < 0.70):
         self.client.moveByMotorPWMsAsync(front_right_pwm=0, rear_left_pwm=1, front_left_pwm=1, rear_right_pwm=0,duration=duration) # Move right
      elif(action < 0.80):
         self.client.moveByMotorPWMsAsync(front_right_pwm=0, rear_left_pwm=1, front_left_pwm=0, rear_right_pwm=1,duration=duration) # Move forward
      elif(action <= 1):
         self.client.moveByMotorPWMsAsync(front_right_pwm=1, rear_left_pwm=0, front_left_pwm=1, rear_right_pwm=0,duration=duration) # Move bacward
